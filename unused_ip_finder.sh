...
# Define Nmap executable path. Leave empty to auto-detect from PATH,
# or set an absolute path if Nmap is installed in a non-standard location.
nmap_path=""
...
# Resolve Nmap executable from PATH if no explicit path is configured
if [ -z "$nmap_path" ]; then
    nmap_path=$(command -v nmap 2>/dev/null || true)
fi

# Check if Nmap path is valid
if [ -z "$nmap_path" ] || [ ! -x "$nmap_path" ]; then
...
"$nmap_path" -Pn -n -sS -p "$ports" --open -T4 "$ip_range" -oG "$nmap_output"
